...
async fn main() {
  ...
  match parse_commands().get_matches().subcommand() {
    Some(("moves", sub_matches)) => {
      ...
      let type_name = sub_matches.get_one::<String>("type_name").map(|s| s.to_owned());
...
fn moves_args() -> Vec<Arg> {
  vec![
    arg!(-p --pokemon <POKEMON_NAME> "The name of the pokemon you want to see moves for").required(true),
